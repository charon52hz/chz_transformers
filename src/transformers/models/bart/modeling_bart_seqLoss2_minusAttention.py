...
class BartMinusAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""
    ...
    def forward(
        ...
        src_len = key_states.size(1)
        attn_weights = torch.bmm(query_states, key_states.transpose(1, 2))
        attn_weights_neg = -1 * attn_weights  ######################chz
        ...
        if attention_mask is not None:
            ...
            attn_weights_neg = attn_weights_neg.view(bsz, self.num_heads, tgt_len,src_len) + attention_mask  ######################chz
            attn_weights_neg = attn_weights_neg.view(bsz * self.num_heads, tgt_len, src_len)  ######################chz

        attn_weights = nn.functional.softmax(attn_weights, dim=-1)
        attn_weights_neg = nn.functional.softmax(attn_weights_neg, dim=-1)  ######################chz

        attn_weights = attn_weights - attn_weights_neg  ######################chz
...
BART_ATTENTION_CLASSES = {
    ...
    "minus_attention": BartMinusAttention,
}
...
class BartDecoderLayer(nn.Module):
    def __init__(self, config: BartConfig):
        ...
        attn_type = "flash_attention_2" if getattr(config, "_flash_attn_2_enabled", False) else "minus_attention"
        self.self_attn = BART_ATTENTION_CLASSES[attn_type](
            embed_dim=self.embed_dim,
