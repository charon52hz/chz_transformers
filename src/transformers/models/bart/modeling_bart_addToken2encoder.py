...
class BartEncoder(BartPreTrainedModel):
    ...
    def forward(
        ...
        input_length = torch.ne(input_ids, 40000).sum(-1) - 3
        input_matrix = torch.ne(input_ids, 40000).int().float()
        actually_input_embeds = torch.mul(inputs_embeds, input_matrix.unsqueeze(-1))
        actually_input_embeds = actually_input_embeds[:, 3:, :]
        ...
        def find_most_similar_vectors(batch_vectors, target_vectors):
            ...
            # 计算批次向量与目标向量的余弦相似度
            similarities = cosine_similarity(batch_vectors, target_vectors.unsqueeze(0), dim=2)
            # 找到每个批次向量最相似的目标向量索引
        ...
        extra_embed = most_similar_vectors.unsqueeze(1)

        input_left_mean = input_left_mean[1:, :, :]
        input_right_mean = input_right_mean[1:, :, :]
        most_similar_left_vectors = find_most_similar_vectors(input_left_mean, label_mean).unsqueeze(1)
        most_similar_right_vectors = find_most_similar_vectors(input_right_mean, label_mean).unsqueeze(1)
        inputs_embeds = torch.cat((extra_embed, most_similar_left_vectors, most_similar_right_vectors, actually_input_embeds), dim=1)
        ##########################
