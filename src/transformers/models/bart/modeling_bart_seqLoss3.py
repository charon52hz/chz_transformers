...
class BartForConditionalGeneration(BartPreTrainedModel):
    ...
    def forward(
        ...
        similarity12 = torch.nn.functional.cosine_similarity(right_average_decoder_output, right_average_decoder_input, dim=-1)
            ### 1/4 sentence
        quarter_index = decoder_input_ids.shape[1] // 4
        average_decoder_output1 = torch.mean(shared_decoder_outputs[:, :quarter_index, :], dim=1, keepdim=True)
        average_decoder_output2 = torch.mean(shared_decoder_outputs[:, quarter_index:half_index, :], dim=1, keepdim=True)
        average_decoder_output3 = torch.mean(shared_decoder_outputs[:, half_index:half_index+quarter_index, :], dim=1, keepdim=True)
        average_decoder_output4 = torch.mean(shared_decoder_outputs[:, half_index+quarter_index:, :], dim=1, keepdim=True)

        average_decoder_input1 = torch.mean(shared_decoder_inputs[:, :quarter_index, :], dim=1, keepdim=True)
        average_decoder_input2 = torch.mean(shared_decoder_inputs[:, quarter_index:half_index, :], dim=1, keepdim=True)
        average_decoder_input3 = torch.mean(shared_decoder_inputs[:, half_index:half_index+quarter_index, :], dim=1, keepdim=True)
        average_decoder_input4 = torch.mean(shared_decoder_inputs[:, half_index+quarter_index:, :], dim=1, keepdim=True)

        similarity21 = torch.nn.functional.cosine_similarity(average_decoder_output1, average_decoder_input1, dim=-1)
        similarity22 = torch.nn.functional.cosine_similarity(average_decoder_output2, average_decoder_input2, dim=-1)
        similarity23 = torch.nn.functional.cosine_similarity(average_decoder_output3, average_decoder_input3, dim=-1)
        similarity24 = torch.nn.functional.cosine_similarity(average_decoder_output4, average_decoder_input4, dim=-1)
        #####################
        ...
        if labels is not None:
            ...
            similarity_loss21 = torch.nn.functional.mse_loss(similarity21, similarity_target)
            similarity_loss22 = torch.nn.functional.mse_loss(similarity22, similarity_target)
            similarity_loss23 = torch.nn.functional.mse_loss(similarity23, similarity_target)
            similarity_loss24 = torch.nn.functional.mse_loss(similarity24, similarity_target)

            masked_lm_loss = masked_lm_loss + similarity_loss + \
            similarity_loss11 + similarity_loss12 + \
            similarity_loss21 + similarity_loss22 + similarity_loss23 + similarity_loss24
